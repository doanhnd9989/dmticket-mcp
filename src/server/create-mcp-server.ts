...
export const createMcpServer = (
  ...
  const mcpServer = new McpServer(
    {
      name: env.DMTICKET_MCP_SERVER_NAME ?? packageName,
      version: packageVersion,
    },
    { instructions: env.DMTICKET_MCP_SERVER_INSTRUCTIONS },
  )

  const getApiKey = (): string =>
    options?.getApiKey?.().trim() || env.DMTICKET_API_KEY
    ...
    if (!apiKey) {
      return {
        ...
        content: [
          {
            type: "text",
            text: "No workspace token configured. Set DMTICKET_API_KEY in the server environment or pass the token via the ?workspace_token= URL query parameter.",
          },
        ],
      }
    }
