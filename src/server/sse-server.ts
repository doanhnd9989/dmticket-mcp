...
const makeGetApiKey = (req: IncomingMessage): (() => string) => {
  const token = getApiTokenFromRequest(req) || env.DMTICKET_API_KEY
  return () => token
}
...
const handleSseRequest = async (
  ...
  if (!sessionId) {
    ...
    const transport = new SSEServerTransport(
      env.DMTICKET_MCP_MESSAGES_PATH,
      res,
    )
...
export const runSseServer = async (
  ...
  const httpServer = createServer(async (req, res) => {
    ...
    if (url.pathname === env.DMTICKET_MCP_SSE_PATH) {
      await handleSseRequest(req, res, createMcpServer)
      return
    }

    if (url.pathname === env.DMTICKET_MCP_MESSAGES_PATH) {
  ...
  await new Promise<void>((resolve) => {
    httpServer.listen(env.DMTICKET_MCP_PORT, env.DMTICKET_MCP_HOST, resolve)
  })

  console.error(
    `MCP Server running on http://${env.DMTICKET_MCP_HOST}:${env.DMTICKET_MCP_PORT}${env.DMTICKET_MCP_SSE_PATH}`,
  )
}
